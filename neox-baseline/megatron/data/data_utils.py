...
def build_loaded_iterators(neox_args):
    """Build iterators for preprocessed loaded datasets (e.g., the Pile 1k batches)"""

    (train_dataloader, valid_dataloader, test_dataloader) = (None, None, None)

    print_rank_0("> building 'loaded' train, validation, and test datasets ...")

    # Hard-code the number of batches, batch size, and sequence length
    num_batches = 1000
    batch_size = 1024
    seq_len = 2049  # 注意这里！是 2049，不是 2048！因为 bin 文件每行是 input+label

    train_ds = LoadedDataset(neox_args.train_data_paths[0], num_batches, batch_size, seq_len)
    train_dataloader = make_data_loader(train_ds, neox_args=neox_args)
    ...
    train_data_iterator = iter(train_dataloader) if train_dataloader is not None else None
    valid_data_iterator = None
    test_data_iterator = None
