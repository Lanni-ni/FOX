...
class FoXAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, layer_past=None):
        ...
        fgate = self.forget_gate(hidden_states)

        
        def shape(x):
            return x.permute(1, 0, 2).view(batch_size, seq_len, self.num_heads, self.head_dim)

        q = shape(q)
        k = shape(k)
        v = shape(v)
        fgate = fgate.permute(1, 0, 2)  

        
        log_fgate = F.logsigmoid(fgate.float())  

      
        out = forgetting_attention(q, k, v, log_fgate)

    
        out = out.permute(2, 0, 1, 3).contiguous().view(seq_len, batch_size, self.hidden_size)
